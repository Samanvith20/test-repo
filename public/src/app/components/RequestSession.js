...
const RequestSession = ({ tutorId, setShowModal }) => {
  ...
  //     return;
  //   }
  //   const toastId = toast.loading("Sending Session Request...");
  ...
  //   try {
  //     const response = await fetch("/api/student/request-session", {
  //       method: "POST",
  //       headers: {
  ...
  const handleSendRequest = async () => {
    ...
    try {
      ...
      const year = selectedDateObj.rawDate.year;

      const payload = {
        studentUsername: session?.username,
        tutorId: tutorId,
        timeslot: {
          date: `${selectedSlot.date}, ${year}`,
          time: `${selectedSlot.from} - ${selectedSlot.to}`,
          timezone: tutor?.tutorDetails?.timezone,
        },
        classDescription: detailsOfClass,
        subjectDetails: {
          subject: subject,
        ...
        classDuration: duration === 60 ? "1 Hour" : "30 Minutes",
        selectedTime: selectedDateInTutorTimezone,
      };
      ...
      if (response.ok) {
        ...
        const data = await response.json();
        toast.error(data.message || "Failed to send the request.", {
          id: toastId,
        });
      }
    } catch (error) {
